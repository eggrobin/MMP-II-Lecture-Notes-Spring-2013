\newcommand{\FigureSimplePendulumConfigurationSpace}{%
\begin{tikzpicture}
%\draw [fill, pattern = north east lines, draw = none] (-1.75, 0) -- ++(3.5, 0) -- ++(45:.25) -- ++(-3.5, 0) -- cycle;
\useasboundingbox (-2.5, -4) rectangle (2.5, 0);
\foreach \x in {-.5,-.45,...,.51}
  \draw (\x, 0) -- ++(45:.125);
\draw (-.5, 0) -- (.5, 0);
\coordinate (P) at (0, 0);
\coordinate (I) at (0, -4);
\node at (-65 : 4) [name=m, circle, inner sep = .25 cm, draw] {};
\node at (P) [name=pivot, label={[label distance=1cm]-87:$q$}] {};
\draw (P) -- (m);
\draw [->] (-90 : 1) arc (-90 : -65 : 1);
\draw [dashdotted] (I) -- (P);
\end{tikzpicture}%
}

\newcommand{\FigureDoublePendulumConfigurationSpace}{%
\begin{tikzpicture}
%\draw [fill, pattern = north east lines, draw = none] (-1.75, 0) -- ++(3.5, 0) -- ++(45:.25) -- ++(-3.5, 0) -- cycle;
%\foreach \x in {-1.75,-1.65,...,1.76}
%  \draw (\x, 0) -- ++(45:.25);
%\draw (-1.75, 0) -- (1.75, 0);
\useasboundingbox (-2.5, -4) rectangle (2.5, 0);
\foreach \x in {-.5,-.45,...,.51}
  \draw (\x, 0) -- ++(45:.125);
\draw (-.5, 0) -- (.5, 0);
\coordinate (P) at (0, 0);
\coordinate (I) at (0, -4);
\coordinate (M1) at (-60 : 2);
\coordinate (M2) at ($(M1) + (-150 : 2)$);
\coordinate (D) at ($(M1)-(P)!(M1)!(I)$);
\coordinate (I2) at ($(I)+(D)$);
\node at (M1) [name=m1, circle, inner sep = .125 cm, label={[label distance=1cm]-100:$q_2$}, draw] {};
\node at (M2) [name=m2, circle, inner sep = .125 cm, draw] {};
\node at (P) [name=pivot, label={[label distance=1cm]-87:$q_1$}] {};
\draw (P) -- (m1) -- (m2);
\draw [->] (-90 : 1) arc (-90 : -60 : 1);
\draw [->] ($(M1)+(0, -1)$) arc (-90 : -150 : 1);
\draw [dashdotted] (I) -- (P);
\draw [dashdotted] (I2) -- (m1);
\end{tikzpicture}
}

\newcommand{\FigureComplexThingThatYouCantVisualise}{%
\begin{tikzpicture}[scale=.85]
\coordinate (I) at (.5,.5);
\coordinate (Tin) at (45:1);
\coordinate (Tout) at (45:-1);
\coordinate (P) at (0,-.75);
\coordinate (Pin) at (45:1);
\coordinate (Pout) at (45:-3);
\draw plot [smooth cycle] coordinates {(1.75,1.75) (0,2) (-1,1) (-2.5,3) (-1.5,0) (-2,-3) (-.5,-1.5) (2,-3) (1,0) (3,.5)};
\node at (I) [name=i, circle, inner sep = .05 cm, fill=black, label={[label distance=0.25cm]15:$\Identity$}, draw] {};
\node at ($(I)+(Tout)$) [name=t, label={[label distance = 0.1 cm]90:$\TimeDerivative\matgg\of 0$}] {};
\draw [->] (I) -- +(Tout);
\node at (P) [name=p, label={0:$\matgg$}] {};
\draw (-.75,.75) .. controls +(0:1) and +(Tin)  .. (I);
\draw [->] (I) .. controls +(Tout) and +(Pin) .. (P);
\draw (P) .. controls +(Pout) and +(120:1) .. (-1,-.75);
\node at (1, -2) {$G$};
\end{tikzpicture}%
}

\newcommand{\FigureRotationAdjointRep}{%
\begin{tikzpicture}
\coordinate (O)  at (0,0);
\coordinate (N)  at (90:1);
\coordinate (NE) at (30:1);
\coordinate (S)  at ($(O)-(N)$);
\coordinate (SE) at ($(S)+(NE)$);
\coordinate (E)  at ($(SE)+(NE)$);
\coordinate (W)  at ($(O)-(E)$);
\coordinate (NW) at ($(O)-(SE)$);
\coordinate (SW) at ($(O)-(NE)$);
\coordinate (a)  at ($2*(N)$);
\coordinate (b)  at ({cos(160)*sqrt(3)/sqrt(2)},{sin(160)/sqrt(2)});
\coordinate (c)  at ({cos(250)*sqrt(3)/sqrt(2)},{sin(250)/sqrt(2)});
\draw[gray, very thin] (N) -- (W) -- (S) -- (E) -- cycle;
\draw[gray, very thin] (NE) -- (SW);
\draw[gray, very thin] (NW) -- (SE);
\draw[->] (O) -- (a);
\draw ($.2*(SE)$) -- ($.2*(NE)$) -- ($.2*(N)$);
\node at (b) [name=i, circle, inner sep = .01 cm, fill=black, label={[label distance=0.25cm]170:$\matb$}, draw] {};
\node at (c) [name=i, circle, inner sep = .01 cm, fill=black, label={[label distance=0.25cm]210:$\AdjointRep\of{\E^{\gf\mata}}\matb$}, draw] {};
\node at (a) [name=i, label={[label distance=0.25cm]250:$\mata$}] {};
\node at (O) [label={[label distance=0.25cm]190:$\ga$}] {};
%\draw (O) ellipse ({sqrt(3)/sqrt(2)} and {1/sqrt(2)});
\draw[->] (O) ($.2*(b)$) arc (160:250:{.2*sqrt(3)/sqrt(2)} and {.2/sqrt(2)});
\draw[gray] (b) -- (O) -- (c);
\end{tikzpicture}%
}

\newcommand{\FigureRotationRThree}{%
\begin{tikzpicture}
\coordinate (O)  at (0,0);
\coordinate (N)  at (90:1);
\coordinate (NE) at (30:1);
\coordinate (S)  at ($(O)-(N)$);
\coordinate (SE) at ($(S)+(NE)$);
\coordinate (E)  at ($(SE)+(NE)$);
\coordinate (W)  at ($(O)-(E)$);
\coordinate (NW) at ($(O)-(SE)$);
\coordinate (SW) at ($(O)-(NE)$);
\coordinate (a)  at ($2*(N)$);
\coordinate (b)  at ({cos(160)*sqrt(3)/sqrt(2)},{sin(160)/sqrt(2)});
\coordinate (c)  at ({cos(250)*sqrt(3)/sqrt(2)},{sin(250)/sqrt(2)});
\draw[gray, very thin] (N) -- (W) -- (S) -- (E) -- cycle;
\draw[gray, very thin] (NE) -- (SW);
\draw[gray, very thin] (NW) -- (SE);
\draw[->] (O) -- (a);
\draw ($.2*(SE)$) -- ($.2*(NE)$) -- ($.2*(N)$);
\node at (b) [name=i, circle, inner sep = .01 cm, fill=black, label={[label distance=0.25cm]90:$\Transpose{\tuple{x_2, -x_1, 0}}$}, draw] {};
\node at (c) [name=i, circle, inner sep = .01 cm, fill=black, label={[label distance=0.25cm]270:$\E^{\matL_\vx}\Transpose{\tuple{x_2, -x_1, 0}}$}, draw] {};
\node at (a) [name=i, label={[label distance=0.25cm]250:$\vx$}] {};
\node at (O) [label={[label distance=0.25cm]190:$\ga$}] {};
%\draw (O) ellipse ({sqrt(3)/sqrt(2)} and {1/sqrt(2)});
\draw[->] (O) ($.2*(b)$) arc (160:250:{.2*sqrt(3)/sqrt(2)} and {.2/sqrt(2)});
\draw[gray] (b) -- (O) -- (c);
\end{tikzpicture}%
}