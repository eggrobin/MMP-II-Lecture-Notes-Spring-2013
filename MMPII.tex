...
%TODO (eggrobin): switch back to parentheses after checking for proper usage.
\newcommand\of[1]{\squareBrackets*{#1}}
...
% \Lnorm f = \Set{C\geq0}{\abs{f}\leq C \text{almost everywhere}}.
...
% \MIabs is the absolute value of multi-indices, \MIabs \miga = \sum_i \ga_i.
\newcommand\MIabs[1]{\singleBars*{#1}}
...
% \comm is the commutator of operators, \comm A B = AB - BA.
\newcommand\comm[2]{\lsquareCommaRsquare*{#1}{#2}}
...
\newif\ifNeedPascal \NeedPascalfalse
\ifNeedPascal \newcommand{\pascal}[1]{\paretheses*{#1}}
\else \newcommand{\pascal}[1]{#1} \fi
% \Set{x\in X}{P\of x} is the set of x in X such that P\of x is true.
\newcommand{\Set}[2]{\lcurlyBarRcurly{#1}{#2}}
...
% Integral and summation with space indicated below.
...
\begin{document}
    ...
    \begin{proof}
      ...
      \begin{equation*}
       \abs{\ft\gj\of\vk} 
        ...
        \underbrace{\abs{\E^{-\I\scal\vk\vx}}}\diffd\vx \\
      \end{equation*}
      Proof of existence of the integral $\Lnorm[1]\gj$:
      \begin{align*}
        \Int\Rn\abs{\gj\of\vx}\diffd\vx 
        &= \Int\Rn\pa{1+\abs{\vx}^2}^\frac{s}{2}\abs{\gj\of\vx}
        \frac{\diffd\vx}{\pa{1+\abs{\vx}^2}^\frac{s}{2}}  \\
        &\leq \Schwartznorm s\nullmi\gj \Int\Rn\frac{1}{\pa{1+\abs{\vx}^2}^\frac{s}{2}}\diffd\vx\\
      ...
      \begin{align*}
        ...
        &= \ftnrm
          \Int\Rn\gj\of*{\vx}\I\fstpartial{k_j}\E^{-\I\scal\vk\vx} \diffd\vx 
          && \text{as }\fstpartial{k_j}\E^{-\I\scal\vk\vx} =
            \fstpartial{k_j}\E^{-\I\Sum_{r=0}^n k_j x_j}=-\I x_j \E^{-\I\scal\vk\vx}\\
        &= \I\fstpartial{k_j}\ftnrm
          \Int\Rn\gj\of*\vx\E^{-\I\scal\vk\vx} \diffd\vx \\
        &= \I\fstpartial[\ft\gj]{k_j}\of*\vk
      \end{align*}
      ...
      \begin{align*}
        ...
        &= \Int\Rn\gj\of*\vx\ftnrm\Int\Rn
          ...
          && \commentbox{As $\int_{\R^{2n}}\abs*{\gj\of*\vx
  ...
  \begin{proposition}
  The Fourier transform $\ft\quad$ is a continuous bijective linear map from the Schwartz space $\Schwartz$ to itself. $\rft{\ft{\gj}}=\gj$, where $\rft\gy\of*\vx\coloneqq\ftnrm \int_\Rn\gy\of*\vk\E^{\I\scal\vk\vx}\diffd\vx$, read ``unhat'', ``bird'', ``seagull'', or whatever you like.
  
    \begin{proof}
      \begin{align*}
        \gj\in\Schwartz&\Rightarrow\vx^\miga\pd\vx^\migb\in\Schwartz \\
        \ft{\of*{\vx^\miga\pd\vx^\migb\gj}}\of*\vk
        &=\I^{\abs*\miga}\pd\vk^\miga\ft{\of*{\pd\vx^\migb\gj}}\of*\vk 
          && \text{from the second equality in (\ref{FTDerivative}), applied }\abs*\miga\text{ times.} \\
        &=\I^{\abs*\miga}\pd\vk^\miga\of*{\I^{\abs*\migb}\vk^\migb\ft\gj}\of*\vk \\
        &=\I^{\abs*\miga+\abs*\migb}\pd\vk\of*{\vk^\migb\ft\gj}\of*\vk
      \end{align*}
      ``I should have done it the other way around. [...] Let's try it the other way around.''
      We want:
      \begin{equation*}
        \forall\miga,\migb,\sup_{\vk\in\R^n}\abs*{\vk^\miga\pd\vk^\migb\ft\gj\of*\vk}<\infty
      \end{equation*}
      As that implies $\ft\gj\in\Schwartz$.
      \begin{align*}
        \ft{\of*{\pd\vx^\miga\of*{\vk^\migb\gj}}}\of*\vk
        &=\I^{\abs*\miga+\abs*\migb}\vk^\miga\pd\vk^\migb\ft\gj\of*\vk
        && \commentbox{by applying the first part of (\ref{FTDerivative}) $\abs*\miga
        $ times and the second part $\abs*\migb$ times.}\\
        \forall\miga,\migb,
        \abs*{\vk^\miga\pd\vk^\migb\ft\gj\of*\vk}&\leq\ftnrm\norm*{\pd\vx\of*{\vx^\migb\gj}}_1<\infty
      ...
      At this point, somebody suggests replacing $\sqftnrm\int_\Rn \E^{\I\scal\vk{\pascal{\vx-\vy}}}\diffd\vk$ by $\gd\of*{\vx-\vy}$. ``It's plausible! --- WHY? [...] Ah I said it, so it's plausible.'' However, doing this would also lead to some nasty calculations. ``How are we going to do it so fast that you don't get bored, and yet in enough detail that he's convinced? Be sneaky.''
      \begin{align*}
        \rft{\ft{\gj}}
        &=\ftnrm\Int\Rn\ft\gj\of*\vk\E^{\I\scal\vk\vx}
        \underbrace{\lim_{\ge\downarrow 0}\E^{-\ge\frac{\abs*\vk^2}{2}}}_{\mathclap{\substack{1
        \text{ written in}\\\text{some other way}}}}\diffd\vk\\
        &=\lim_{\ge\downarrow 0}\ftnrm\Int\Rn\ft\gj\of*\vk\E^{\I\scal\vk\vx}
        \E^{-\ge\frac{\abs*\vk^2}{2}}&&\commentbox{``Don't worry.'' This is actually a one-liner 
      ...
      \begin{align*}
        \ftnrm\Int\Rn\ft\gj\of*\vk\E^{\I\scal\vk\vx}\E^{-\ge\frac{\abs*\vk^2}{2}}\diffd\vk
        &=\ftnrm\Int\Rn\ftnrm\Int\Rn\gj\of*\vy\E^{-\I\scal\vy\vk}\diffd\vy\:
        \E^{\I\scal\vk\vx}\E^{-\ge\frac{\abs*\vk^2}{2}}\diffd\vk\\
        &=\ftnrm\Int\Rn\gj\of*\vy\ftnrm\Int\Rn\E^{\I\scal\vk{\pascal{\vx-\vy}}}
        \E^{-\ge\frac{\abs*\vk^2}{2}}\diffd\vk\diffd\vy
      \end{align*}
      ...
      \begin{equation*}
        \ftnrm\Int\Rn\E^{-\ge\frac{\abs*\vk^2}{2}}\E^{\I\scal{\pascal{\vy-\vx}}\vk}\diffd\vk
        =\frac{\E^{-\frac{1}{2\ge}\abs*{\vx-\vy}^2}}{\ge^\frac{n}{2}}
      \end{equation*}
      We therefore get:
      \begin{align*}
         \ftnrm\Int\Rn\ft\gj\of*\vk\E^{\I\scal\vk\vx}\E^{-\ge\frac{\abs*\vk^2}{2}}\diffd\vk
         &=\ftnrm\Int\Rn\gj\of*\vy\ftnrm\Int\Rn\E^{\I\scal\vk{\pascal{\vx-\vy}}}
        \E^{-\ge\frac{\abs*\vk^2}{2}}\diffd\vk\diffd\vy\\
        &=\ftnrm\Int\Rn\gj\of*\vy\frac{\E^{-\frac{1}{2\ge}\abs*{\vx-\vy}^2}}
        {\ge^\frac{n}{2}}\diffd\vy \\
        &=\ftnrm\Int\Rn\gj\of*{\vy\sqrt\ge+\vx}\frac{\E^{-\frac{1}{2\ge}\abs*{\vy\sqrt\ge}^2}}
        {\ge^\frac{n}{2}}\diffd\of*{\vy\sqrt\ge} && \text{``Epsilons everywhere!''}\\    
        &=\ftnrm\Int\Rn\gj\of*{\vx+\vy\sqrt\ge}\E^{-\frac{1}{2}\abs*\vy^2}\diffd\vy
        && \text{``Now we can paint.''}\\
        &\overset{\ge\downarrow 0}{\rightarrow}
        \ftnrm\gj\of*\vx\Int\Rn\E^{-\frac{1}{2}\abs*\vy^2}\diffd\vy =\gj\of*\vx.
      \end{align*}
    \end{proof}
  \end{proposition}
  ...
  \begin{equation*}
    \ft{\E^{-\frac{1}{2}\abs*\vx^2}}=\E^{-\frac{1}{2}\abs*\vx^2},
  \end{equation*}
  ...
  Let $H \coloneqq \frac{1}{2}\of*{-\nthderiv{x}{2}+x^2-1}$. Then $\comm H {\ft\quad}=0$. We multiplied by $\frac{1}{2}$ so that it looks even more like a harmonic potential (it is the Hamiltonian of the quantum harmonic oscillator). Why did we add $-1$? Define
  \begin{align*}
