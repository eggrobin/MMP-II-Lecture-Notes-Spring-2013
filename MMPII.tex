...
The dual space of $\Rn$ is linked to the dual group in the following way: we have $\DualGroup\Rn\DefineAs\setst{f\in\Functions{\Rn}{\UnitaryGroup\of 1}}{f\of{\vx+\vy}=f\of \vx f\of \vy}$. Those functions are the $\E^{\I\scal{\vk}{\pascal\placeholder}}$, where $\scal{\vk}{\pascal\placeholder}\in\dual{\pa\Rn}$. Moreover, $\ft\gj\of\vk = \ftnrm \int\Rn \gj\of\vx \conj{\E^{\I\scal{\vk}{\vx}}}\diffd \vx$, that is, $\LTwoInner{\E^{\I\scal{\vk}{\pascal\placeholder}}}{\gj}$, which is the infinite analogue to $\sum{u\in A}\conj f\of u g\of u$, the inner product on $\Lspace[2]\of A$.

The way a bourgeois mathematician thinks of the phase space is as the cotangent bundle $\Rn\times\dual{\pa\Rn}$.
...
\paragraph*{Examples from Raisa Galimova's exercise class}
\begingroup
\newcommand{\FunctionSpace}{\mathcal F}
\newcommand{\LeftAction}[2]{#1 #2}
Let $G$ be a group acting on the left on a measure space $\tuple{X, \gS, \gm}$ --- where $X$ is a set, $\gm$ is a measure, $\gS\Subset\PowerSet X$ the $\gs$-algebra of $\gm$-measurable subsets --- by $\FunctionSpec f G {\Functions X X}, \FunctionBody g {f_g}$ and define the space of square-integrable functions 
\begin{align*}
  \Lspace[2]\of X &\DefineAs \setst{\gj\in\Functions \C X}{\int X \abs{\gj\of x}^2 \diffd \gm < \infty}
\intertext{associated with $X$. Note that if $X$ is finite, and $\gm$ is the counting measure,}
  \int X \abs{\gj\of x}^2 \diffd \gm &= \sum {g\in X} \abs{\gj\of x}^2 &= \LTwoInner\gj\gj
\intertext{so we recover the definition from the lecture for a finite group $X$. If $X=\Rn$, and $\gm$ is the Lebesgue measure,}
  \int X \abs{\gj\of x}^2 \diffd \gm &= \int \Rn \abs{\gj\of \vx}^2 \diffd\vx &= \LTwoInner\gj\gj = \Lnorm[2]\gj
\end{align*}
so again we recover the definition we gave for the inner product on $\Schwartz\Subset\Lspace[2]\of\Rn$.
\begin{enumerate}
\item If $G=\UnitSphere 1=X$ is the ``circle group,'' acting on itself by addition (here we use the definition $\UnitSphere 1 = \QuotientGroup\R {2\Pi\Z}$), $\FunctionSpace \of X = \Lspace[2]\of{\UnitSphere 1}$ decomposes in a way which is dictated by Fourier series, i.e., ;
\item If $G=\R=X$, which acts by addition, $\FunctionSpace \of X$ decomposes to the Fourier transform;
\item If $G=\SpecialOrthogonalGroup\of 3$ acts by rotations on $X=\UnitSphere 2$, we get spherical harmonics.
\end{enumerate}
\endgroup
