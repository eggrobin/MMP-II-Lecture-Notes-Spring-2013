...
%TODO(eggrobin): the big partial derivative is nonsemantic.
  ...
  \begin{definition}[Locally integrable functions]
    \begin{equation*}
      \LspaceLoc[1]\of\Rn \DefineAs \setst{\funcspec f \Rn \C}{\forall \vx \in \Rn, \exists \ge > 0, \int{\OpenBall \ge \vx}\abs{f\of\vy} \diffd{\vy} < \infty}
    \end{equation*}
  \end{definition}
...
We define $\SignDistrib\of\gj \DefineAs \int{-\infty}[\infty] \Sign\of{x} \gj\of{x} \diffd{x}$ and write $\derivop x \mathrm{Abs} = \SignDistrib$. This corresponds to $\derivop x \abs{x} = \Sign\of{x}$ in physicist-speak.
  ...
  \begin{equation*}
  \SchwartzInner \gy \gj \DefineAs \int\Rn \conj\gy\of\vx \gj\of\vx \diffd\vx .
  \end{equation*}
  \end{definition}
...
The dual space of $\Rn$ is linked to the dual group in the following way: we have $\DualGroup\Rn\DefineAs\setst{f\in\Functions{\Rn}{\UnitaryGroup\of 1}}{f\of{\vx+\vy}=f\of \vx f\of \vy}$. Those functions are the $\E^{\I\scal{\vk}{\pascal\placeholder}}$, where $\scal{\vk}{\pascal\placeholder}\in\dual{\pa\Rn}$. Moreover, $\ft\gj\of\vk = \ftnrm \int\Rn \gj\of\vx \conj{\E^{\I\scal{\vk}{\vx}}}\diffd \vx$, that is, $\SchwartzInner{\E^{\I\scal{\vk}{\pascal\placeholder}}}{\gj}$, which is the infinite analogue to $\sum{u\in A}\conj f\of u g\of u$, the inner product on $\Lspace[2]\of A$.
...
Let $G$ be a group acting on a space $X$ that has a space of --- $\Lspace[2]$, smooth, etc. --- functions $\FunctionSpace \of X$ associated to it.
\begin{enumerate}
\item If $G=\UnitSphere 1=X$ is the ``circle group,'' acting on itself by addition (here we use the definition $\UnitSphere 1 = \QuotientGroup\R {2\Pi\Z}$), $\FunctionSpace \of X$ decomposes to Fourier Series;
