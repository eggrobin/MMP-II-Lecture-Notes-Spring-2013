...
The Fourier transform $\ft{\placeholder}$ is a continuous bijective linear map from the Schwartz space $\Schwartz$ to itself. $\rft{\ft{\gj}}=\gj$, where $\rft\gy\of\vx\DefineAs\ftnrm \int\Rn\gy\of\vk\E^{\I\scal\vk\vx}\diffd\vx$, read ``unhat'', ``bird'', ``seagull'', or whatever you like.
  ...
  \begin{align*}
    A &\DefineAs \frac{1}{\sqrt 2}\pa{\derivop{x} + x}\text{,}\\
    \adj A &\DefineAs \frac{1}{\sqrt 2}\pa{-\derivop{x} + x}\text{.}
  \end{align*}
...
\begin{align*}
  ...
  &= - \LTwoInner {P\gj} \gy \text{,}&&\parbox{7cm}{$\diff {-\infty}{\infty}{\conj\gj\gy} = 0$  as $\gj\in\SchwartzSpace\of\R$, $\gy\in\SchwartzSpace\of\R$, and therefore $\conj\gj\gy\in\SchwartzSpace\of\R$.}
\end{align*}
...
We defined:
\begin{align*}
A&\DefineAs\frac{1}{\sqrt{2}}\pa{x+\derivop x}\text,\\
\adj{A}&\DefineAs\frac{1}{\sqrt{2}}\pa{x+\derivop x}\text.
\end{align*}
...
&=\LTwoInner {\adj A \gy}\gj\text.
\end{align*}

Write 
\begin{align*}
A_x&\DefineAs\frac{1}{\sqrt{2}}\pa{x+\derivop x}\text,\\
\adj{A_x}&\DefineAs\frac{1}{\sqrt{2}}\pa{x+\derivop x}\text{,}
\end{align*}
and similarly for $k$ instead of $x$.
Observe:
\begin{align*}
\FT\of {A_x \gj}\of k &= \I A_k \ft\gj\of k\text,\\
\FT\of {\adj{A_x} \gj}\of k &= \I \adj{A_k} \ft\gj\of k\text,\\
...
Ah_0&=0\text, \\
\ft{h_0}&= h_0\text,\\
\ft{\adj {A_x} h_0}\of k &=\pa{-\I}\adj{A_k} h_0\of k\text{ using the vital lemma,}\\
\FT\of{\pa{\adj{A_x}}^n h_0}\of k &= \pa{-\I} \adj{A_k}\FT\of{\pa{\adj A_x}^{n-1}h_0}\of k
= \pa{-\I}^n \pa{\adj{A_k}}^n h_0\of k\text.
\end{align*}
In other words, the $\pa{\adj A}^n h_0$ --- with an $\adj A$, not an $A$; a dagger can be the difference between life and death! --- are eigenfunctions of the the Fourier transform. We will need to prove that these form a complete set, and we will have an orthogonal basis of $\SchwartzSpace\of\R$.
