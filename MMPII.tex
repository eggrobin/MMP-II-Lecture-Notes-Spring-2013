...
\officialNotes
\begingroup
\newcommand{\VelocityAlgebra}{\LieAlgebra{g}}
The dual group, also know as Pontryagin dual, only works for abelian groups.
Indeed, if the space of generalised coordinates of a physical system $\vq$ is a Lie group $G$, i.e., a group which is also a differentiable manifold, then the generalised velocity $\TimeDerivative\vq$ at $\vq$ lies in the tangent space $\TangentSpace{\vq}G\stackrel{\diffd\gj\of\vq}\Isomorphic\TangentSpace{\Identity}G$, where the linear isomorphism $\diffd\gj\of\vq$ is canonical, and is given by the diffeomorphism $\gj$ of composition with $\vq^{-1}$, $\FunctionSpec\gj G G, \FunctionBody\vx {\vq^{-1}\vx}$. The ordered pair $\tuple{\vq,\TimeDerivative\vq}$ lies in the tangent bundle, that is,
\begin{equation*}
\tuple{\vq,\TimeDerivative\vq} \in \TangentBundle G \DefineAs \UnionOver{\vq \in G} \TangentSpace{\vq}G
\end{equation*}
\endgroup

The dual space of $\Rn$ is linked to the dual group in the following way: we have $\PontryaginDual\Rn\DefineAs\setst{\FunctionSpec f{\Rn}{\UnitaryGroup\of 1}}{f\of{\vx+\vy}=f\of \vx f\of \vy}$. Those functions are the $\E^{\I\scal{\vk}{}}$, where $\scal{\vk}{}\in\Dual{\pa\Rn}, \scal{\vk}{} \DefineAs \FunctionBody \vx {\scal \vk\vx}$. Moreover, $\ft\gj\of\vk = \ftnrm \int\Rn \gj\of\vx \conj{\E^{\I\scal{\vk}{\vx}}}\diffd \vx$, that is, $\LTwoInner{\E^{\I\scal{\vk}{}}}{\gj}$, which is the continuous analogue to $\LTwoInner f g =\sum{u\in A}\conj f\of u g\of u$, the inner product on $\Lspace[2]\of A$.

The way a bourgeois mathematician thinks of the classical phase space $\Rn\times\PontryaginDual\Rn$ is as the cotangent bundle $\Rn\times\Dual{\pa\Rn}$.
...
\end{equation*}
To summarise, the translation action $\gr$ is a unitary representation of $\UnitSphere 1$ on $\Lspace[2]\of{\UnitSphere 1}$ with respect to the inner product $\LTwoInner \placeholder\placeholder$, it decomposes in the irreducible representations $V_j$ which are the spans of individual elements $\E^{\I j \placeholder}$ of $\PontryaginDual{\UnitSphere 1}$ and $\ft\placeholder\of{j}$ is the projector onto $V_j$. Since $\PontryaginDual {\UnitSphere 1} \Isomorphic \Z$, and the elements of $\PontryaginDual {\UnitSphere 1}$ are the basis onto which functions are projected by the Fourier transform, it makes sense to think of the projector as $\ft\placeholder\of{\E^{\I j \placeholder}}$ instead, with $\ft\gj\of{\gc}\DefineAs\LTwoInner\gc\gj$ for $\gc\in\PontryaginDual {\UnitSphere 1}$. This notation has the advantage of not relying on the identification of $\Z$ with $\PontryaginDual{\UnitSphere 1}$, which is not unique.
...
\int\Rn\ft\gj\E^{\I\scal\vx{}}\diffd \PontryaginDual\gm = \gj\of\vx, \quad
\text{where $\PontryaginDual\gm$ is the dual measure,\footnotemark}
\end{equation*}
\footnotetext{Here the dual measure $\PontryaginDual\gm$ is the measure we need to get the
