...
Indeed, if the space of generalised coordinates of a physical system $\vq$ is a Lie group $G$, i.e., a group which is also a differentiable manifold, then the generalised velocity $\TimeDerivative\vq$ at $\vq$ lies in the tangent space $\TangentSpace{\vq}G\Isomorphic\TangentSpace{\Identity}G$, where the isomorphism $\gj$ is given by the diffeomorphism by composition $\vq^{-1}$ in $G$ $\FunctionBody\vx {\vq^{-1}\vx}$, namely $\gj\TangentSpace{\vq^{-1}}G\Isomorphic\TangentSpace{\Identity}G$ is canonic. ordered pair $\tuple{\vq,\TimeDerivative\vq}$ lies in the tangent bundle, that is,
\begin{equation*}
